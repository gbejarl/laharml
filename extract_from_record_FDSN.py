...
client_password = "NmjW6vCnoRiZIDyh"
...
if client_username == "" or client_password == "":
    ...
    client = Client(client_name,user=client_username,password=client_password)
...
for i in range(len(dtm1)):
    ...
    dl = FDSN_download_extract(client,network,station,location,channel,starttime,endtime,features_class,extended,window=window_length,overlap=window_overlap,decimate=decimation_factor,min_freq=minimum_frequency,max_freq=maximum_frequency,plot=True)
...
while starttime < dt2:
    ...
    try:
        unclassified_data_frame,st = FDSN_download_extract(client,network,station,location,channel,starttime,endtime,features,window=window_length,overlap=window_overlap,keep=True)
